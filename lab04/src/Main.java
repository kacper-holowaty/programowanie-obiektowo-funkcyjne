...
public class Main {
    private static void menu() {
        ...
        System.out.println("6 - dodanie 7 spotkań na raz na wtorek (dzień numer 2)");
        System.out.println("7 - zakończenie działania programu\n");
    }
    ...
    private static void opcja3 (Kalendarz kalendarz, Scanner scan) {
        ...
        int dzienTygodnia = scan.nextInt();
        if (dzienTygodnia>=1 && dzienTygodnia < 8) {
            ArrayList<Spotkanie> spotkania = kalendarz.zwrocSpotkania(dzienTygodnia);
            for (int i = 0; i < spotkania.size(); i++) {
                System.out.println(spotkania.get(i).wyswietlObiekt());
            }
        }
        else {
            System.out.println("Niepoprawny dzień tygodnia.");
    ...
    private static void opcja4 (Kalendarz kalendarz, Scanner scan) {
        ...
        if (dzienTygodnia>=1 && dzienTygodnia < 8) {
            ...
            String priorytet = scan.nextLine();
            ArrayList<Spotkanie> spotkania = kalendarz.zwrocSpotkaniaOPodanymPriorytecie(dzienTygodnia, priorytet);
            for (int i = 0; i < spotkania.size(); i++) {
                System.out.println(spotkania.get(i).wyswietlObiekt());
            }
        }
    ...
    private static void opcja5 (Kalendarz kalendarz, Scanner scan) {
        ...
        if (dzienTygodnia>=1 && dzienTygodnia < 8) {
            ...
            int minute = scan.nextInt();
            LocalTime godzinaSpotkania = LocalTime.of(hour, minute);
            ArrayList<Spotkanie> spotkania = kalendarz.zwrocSpotkaniaZaczynajaceSie(dzienTygodnia, godzinaSpotkania);
            for (int i = 0; i < spotkania.size(); i++) {
                System.out.println(spotkania.get(i).wyswietlObiekt());
            }
        }
    ...
    private static void opcja6(Kalendarz kalendarz) {
    ...
    public static void main(String[] args) {
        ...
        while (uruchomiony) {
            ...
            switch (opcja) {
                ...
                case 6 -> opcja6(kalendarz);
                case 7 -> uruchomiony = false;
